...
module.exports = {
    ...
    utils: {
        prefix: (prefix) => `Mano dabartinis priešdėlis je \`${prefix}\`\nPouži \`${prefix}help\` pre viac pomoci.`,
...
:pencil: **| Teraz napíš správu odchodu... :pencil2:**`,
    ...
    setlang: {
        invalid: () => `${emojis.error} | Jūs turite rašyti galiomą kalbą!\n\n:flag_fr: Français (\`fr\`)\n:flag_gb: English (\`en\`)\n:flag_tr: Türkçe (\`tr\`)\n:flag_ru: Русский (\`ru\`)\n:flag_jp: 日本語 (\`jp\`)\n:flag_in: हिंदी (\`hi\`)\n:flag_pt: Português (\`pt\`)\n:flag_br: Português Brasileiro (\`br\`)\n:flag_es: Español (\`es\`)\n:flag_de: Deutsch (\`de\`)\n:flag_it: Italiano (\`it\`)\n:flag_pl: Polski (\`pl\`)\n:flag_nl: Nederlands (\`nl\`)\n:flag_se: Svenska (\`sv\`)\n:flag_no: Norsk (\`no\`)\n:flag_dk: Dansk (\`da\`)\n:flag_fi: Suomi (\`fi\`)\n:flag_gr: Ελληνικά (\`el\`)\n:flag_cz: Čeština (\`cs\`)\n:flag_hu: Magyar (\`hu\`)\n:flag_ro: Română (\`ro\`)\n:flag_rs: Српски (\`sr\`)\n:flag_hr: Hrvatski (\`hr\`)\n:flag_bg: Български (\`bg\`)\n:flag_ua: Українська (\`uk\`)\n:flag_sk: Slovenčina (\`sk\`)\n:flag_si: Slovenščina (\`sl\`)\n:flag_al: Shqip (\`sq\`)\n:flag_is: Íslenska (\`is\`)\n:flag_by: Беларуская (\`be\`)\n:flag_lt: Lietuvių (\`lt\`)`,
        success: () => `${emojis.success} | :flag_lt: Serverio kalba dabar yra lietuvių!`
    },
    graph: {
        title: (server, days) => `Prisijungimai prie ${server} per paskutinius ${days} dienas`,
        content: (total, percent, from, to) => `**${total}** nariai (t.y. **${percent}%** serverio) prisijungė prie serverio nuo ${from} iki ${to}:`,
        invalid: () => `Jūs turite įvesti galiamą dienų skaičių (didesnis nei 1 ir mažesnis nei 1000) rodymui!`,
        months: () =>["Sau","Vas","Kov","Bal","Geg","Bir","Lie","Rūp","Ruo","Spa","Lap","Gru"],
    },
    website: {
        doc: {
        ...
        selector: {
            ...
            no: {
                title: () => `Nėra serverio`,
                content: () => `Serveris nerastas. Prašome patikrinti, ar esate prisijungę tinkamu paskyra.`
            }
        },
        help: {
            title: () => `Pagalba`,
            doc: () => `Dokumentacija`,
            support: () => `Pagalbos serveris`
        },
        forms: {
            buttons: {
                enable: () => `Įjungti žinutes`,
                disable: () => `Išjungti žinutes`,
                update: () => `Atnaujinti žinutes`
            },
            basic: {
                title: () => `Pagrindinė konfigūracija`,
                language: () => `Kalba`,
                prefix: () => `Priešdėlis`,
                update: () => `Atnaujinti`
            },
            join: {
                title: () => `Prisijungimo žinutes`,
                message: {
                    title: () => `Žinutė`,
                    default: () => `{user} prisijungė prie serverio! Jis buvo pakviestas **{inviter.tag}** (kuris turi **{inviter.invites}** pakvietimų).`
                },
                channel: {
                    title: () => `Kanalas`
                },
            ...
            leave: {
                title: () => `Išėjimo žinutes`,
                message: {
                    title: () => `Žinutė`,
                    default: () => `{user.username} paliko serverį. Jis buvo pakviestas **{inviter.tag}** (kuris turi **{inviter.invites}** pakvietimų).`
                },
                channel: {
                    title: () => `Kanalas`
                }
            ...
            joinDM: {
                title: () => `Prisijungimo žinutes DM`,
                premium: () => `Ši funkcija prieinama tik premium serveriams ir partneriams.`,
                message: {
                    title: () => `Žinutė`,
                    default: () => `Sveiki {user} į **{server}! Jūs buvote pakviestas **{inviter.tag}**. Nepamirškite perskaityti serverio taisykles!`,
                }
            }
        }
    },
    removeinvites: {
        loading: {
            all: (prefix) => `${emojis.loading} | Šalinami pakvietimai iš esamo serverio.... Galite juos atkurti naudodami komandą \`${prefix}restore-invites\`!`,
            member: (prefix, member) => `${emojis.loading} | Šalinami **${member.user.tag}** pakvietimai... Galite juos atkurti naudodami komandą \`${prefix}restore-invites ${member.user.tag}\` !`
        },
        title: () => `☄️ Pakvietimai iš naujo inicijuoti`,
        titles: {
            all: (prefix) => `${emojis.success} | Serverio pakvietimai iš naujo inicijuoti! Galite juos atkurti naudodami komandą \`${prefix}restore-invites\`!`,
            member: (prefix, member) => `${emojis.success} | **${member.user.tag}** pakvietimai iš naujo inicijuoti! Galite juos atkurti naudodami komandą \`${prefix}restore-invites ${member.user.tag}\`!`
        }
    },
    restoreinvites: {
        confirmations: {
            all: (prefix, memberCount) => `${emojis.warn} | Ar jūs tikrai norite atkurti serverio pakvietimus? Visi nariai atgaus pakvietimus, kuriuos turėjo prieš paskutinį karto, kai buvo naudota komanda \`${prefix}remove-invite\` (arba 0, jei komanda niekada nebuvo naudota).\n\n:information_source: **Pakvietimų apžvalga**:\nBus atkurti iš viso: **${memberCount.invites}** reguliarūs, **${memberCount.bonus}** bonus, **${memberCount.leaves}** išvyjimai, **${memberCount.fake}** klaidingi.\n\n${emojis.success} Parašykite \`-confirm\` patvirtinimui.\n${emojis.error} Parašykite \`cancel\` atšaukimui.`,
            member: (prefix, member) => `${emojis.warn} | Ar jūs tikrai norite atkurti **${member.user.tag}** pakvietimus? Jis atgaus pakvietimus, kuriuos turėjo prieš paskutinį karto, kai buvo naudota komanda \`${prefix}remove-invites\` (arba 0, jei komanda niekada nebuvo naudota).\n\n:information_source: **Pakvietimų apžvalga**:\nBus atkurti: **${member.data.old_invites}** reguliarūs, **${member.data.old_bonus}** bonus, **${member.data.old_leaves}** išvyjimai, **${member.data.old_fake}** klaidingi.\n\n${emojis.success} Parašykite \`-confirm\` patvirtinimui.\n${emojis.error} Parašykite \`cancel\` atšaukimui.`,
            cancelled: () => `${emojis.error} Atšaukta.`
        },
        loading: {
            all: () => `${emojis.loading} | Serverio pakvietimų atkūrimas....`,
            member: (member) => `${emojis.loading} | **${member.user.tag}** pakvietimų atkūrimas...`
        },
        title: () => `☄️ Pakvietimai atkurti`,
        titles: {
            all: () => `${emojis.success} | Serverio pakvietimai atkurti!`,
            member: (member) => `${emojis.success} | **${member.user.tag}** pakvietimai atkurti!`
        }
    },
    syncinvites: {
        no: () => `${emojis.error} | Nėra pakvietimų sinchronizuoti.`,
        confirmations: {
            all: (inviteCount) => `${emojis.warn} | Ar jūs tikrai norite sinchronizuoti serverio pakvietimus?\n\n:information_source: **Pakvietimų apžvalga**:\nBus atkurti **${Math.round(inviteCount)}** regularūs pakvietimai.\n\n${emojis.success} Parašykite \`-confirm\` patvirtinimui.\n${emojis.error} Parašykite \`cancel\` atšaukimui.`,
            cancelled: () => `${emojis.error} Atšaukta.`
        },
        title: () => `☄️ Pakvietimai sinchronizuoti`,
        titles: {
            all: () => `${emojis.success} | Serverio pakvietimai sinchronizuoti!`
        }
    },
    add: {
        content: (id) => `Galite mane pridėti prie savo serverio spustelėję [čia](https://discordapp.com/oauth2/authorize?client_id=${id}&scope=bot&permissions=2146958847).`,
        requested: (username) => `Paprašė ${username}`
    },
    support: {
        content: (id) => `Galite prisijungti prie mūsų discord serverio spustelėję [čia](https://discord.gg/f7f2P2q).`,
        requested: (username) => `Paprašė ${username}`
    }
};
