...
module.exports = {
    ...
    utils: {
        prefix: (prefix) => `Môj súčasný predpona je \`${prefix}\`\nPouži \`${prefix}help\` pre viac pomoci.`,
...
:pencil: **| Teraz napíš správu odchodu... :pencil2:**`,
    ...
    setlang: {
        invalid: () => `${emojis.error} | Musíš napísať platný jazyk!\n\n:flag_fr: Français (\`fr\`)\n:flag_gb: English (\`en\`)\n:flag_tr: Türkçe (\`tr\`)\n:flag_ru: Русский (\`ru\`)\n:flag_jp: 日本語 (\`jp\`)\n:flag_in: हिंदी (\`hi\`)\n:flag_pt: Português (\`pt\`)\n:flag_br: Português Brasileiro (\`br\`)\n:flag_es: Español (\`es\`)\n:flag_de: Deutsch (\`de\`)\n:flag_it: Italiano (\`it\`)\n:flag_pl: Polski (\`pl\`)\n:flag_nl: Nederlands (\`nl\`)\n:flag_se: Svenska (\`sv\`)\n:flag_no: Norsk (\`no\`)\n:flag_dk: Dansk (\`da\`)\n:flag_fi: Suomi (\`fi\`)\n:flag_gr: Ελληνικά (\`el\`)\n:flag_cz: Čeština (\`cs\`)\n:flag_hu: Magyar (\`hu\`)\n:flag_ro: Română (\`ro\`)\n:flag_rs: Српски (\`sr\`)\n:flag_hr: Hrvatski (\`hr\`)\n:flag_bg: Български (\`bg\`)\n:flag_ua: Українська (\`uk\`)\n:flag_sk: Slovenčina (\`sk\`)\n:flag_si: Slovenščina (\`sl\`)\n:flag_al: Shqip (\`sq\`)\n:flag_is: Íslenska (\`is\`)\n:flag_by: Беларуская (\`be\`)\n:flag_lt: Lietuvių (\`lt\`)`,
        success: () => `${emojis.success} | :flag_sk: Jazyk servera je teraz v Slovenčine!`
    },
    
    graph: {
        title: (server, days) => `Pripojení na ${server} v posledných ${days} dňoch`,
        content: (total, percent, from, to) => `**${total}** členov (t.j. **${percent}%** servera) sa pripojilo na server od ${from} do ${to}:`,
        invalid: () => `Musíš zadať platný počet dní (väčší ako 1 a nižší ako 1000) na zobrazenie!`,
        months: () =>["Jan","Feb","Mar","Apr","Maj","Jún","Júl","Aug","Sep","Okt","Nov","Dec"],
    },
    ...
    website: {
        ...
        selector: {
            ...
            no: {
                title: () => `Žiadny server`,
                content: () => `Server nenájdený. Prosím overi, že si prihlásený s správnym účtom.`
            }
        },
        help: {
            title: () => `Pomoc`,
            doc: () => `Dokumentácia`,
            support: () => `Podporný server`
        },
        forms: {
            buttons: {
                enable: () => `Zapnúť správy`,
                disable: () => `Vypnúť správy`,
                update: () => `Aktualizovať správy`
            },
            basic: {
            ...
            join: {
                title: () => `Správy pripojenia`,
                message: {
                    title: () => `Správa`,
                    default: () => `{user} sa pripojil na server! Bol pozvaný **{inviter.tag}** (ktorý má **{inviter.invites}** pozván).`
                },
                channel: {
                    title: () => `Kanál`
                },
            ...
            leave: {
                title: () => `Správy odchodu`,
                message: {
                    title: () => `Správa`,
                    default: () => `{user.username} opustil server. Bol pozvaný **{inviter.tag}** (ktorý má **{inviter.invites}** pozván).`
                },
                channel: {
                    title: () => `Kanál`
                }
            ...
            joinDM: {
                title: () => `Správy pripojenia v DM`,
                premium: () => `Funkcia dostupná iba pre premium servery a partnerov.`,
                message: {
                    title: () => `Správa`,
                    default: () => `Vitaj {user} na **{server}! Bol si pozvaný **{inviter.tag}**. Nezabudni si prečítať pravidlá servera!`,
                }
            }
        }
    },
    ...
    removeinvites: {
        loading: {
            all: (prefix) => `${emojis.loading} | Odstraňujem pozvánky z aktuálneho servera.... Môžeš ich obnoviť pomocou príkazu \`${prefix}restore-invites\`!`,
            member: (prefix, member) => `${emojis.loading} | Odstraňujem pozvánky **${member.user.tag}**... Môžeš ich obnoviť pomocou príkazu \`${prefix}restore-invites ${member.user.tag}\` !`
        },
        title: () => `☄️ Pozvánky reinicializované`,
        titles: {
            all: (prefix) => `${emojis.success} | Pozvánky servera reinicializované! Môžeš ich obnoviť pomocou príkazu \`${prefix}restore-invites\`!`,
            member: (prefix, member) => `${emojis.success} | Pozvánky **${member.user.tag}** reinicializované! Môžeš ich obnoviť pomocou príkazu \`${prefix}restore-invites ${member.user.tag}\`!`
        }
    },

    restoreinvites: {
        confirmations: {
            all: (prefix, memberCount) => `${emojis.warn} | Si si istý, že chceš obnoviť pozvánky servera? Všetci členovia dostanú pozvánky, ktoré mali pred posledným použitím príkazu \`${prefix}remove-invite\` (alebo 0, ak sa príkaz nikdy nepoužil).\n\n:information_source: **Prehľad pozván**:\nBudú obnovené celkem: **${memberCount.invites}** normálnych, **${memberCount.bonus}** bonusu, **${memberCount.leaves}** odchodov, **${memberCount.fake}** falošných.\n\n${emojis.success} Napíš \`-confirm\` na potvrdenie.\n${emojis.error} Napíš \`cancel\` na zrušenie.`,
            member: (prefix, member) => `${emojis.warn} | Si si istý, že chceš obnoviť pozvánky **${member.user.tag}**? Dostane pozvánky, ktoré mal pred posledným použitím príkazu \`${prefix}remove-invites\` (alebo 0, ak sa príkaz nikdy nepoužil).\n\n:information_source: **Prehľad pozván**:\nBudú obnovené: **${member.data.old_invites}** normálnych, **${member.data.old_bonus}** bonusu, **${member.data.old_leaves}** odchodov, **${member.data.old_fake}** falošných.\n\n${emojis.success} Napíš \`-confirm\` na potvrdenie.\n${emojis.error} Napíš \`cancel\` na zrušenie.`,
            cancelled: () => `${emojis.error} Zrušené.`
        },
        loading: {
            all: () => `${emojis.loading} | Obnovujem pozvánky servera....`,
            member: (member) => `${emojis.loading} | Obnovujem pozvánky **${member.user.tag}**...`
        },
        title: () => `☄️ Pozvánky obnovené`,
        titles: {
            all: () => `${emojis.success} | Pozvánky servera obnovené!`,
            member: (member) => `${emojis.success} | Pozvánky **${member.user.tag}** obnovené!`
        }
    },

    syncinvites: {
        no: () => `${emojis.error} | Žiadne pozvánky na synchronizáciu.`,
        confirmations: {
            all: (inviteCount) => `${emojis.warn} | Si si istý, že chceš synchronizovať pozvánky servera?\n\n:information_source: **Prehľad pozván**:\nBudú obnovené **${Math.round(inviteCount)}** normálne pozvánky.\n\n${emojis.success} Napíš \`-confirm\` na potvrdenie.\n${emojis.error} Napíš \`cancel\` na zrušenie.`,
            cancelled: () => `${emojis.error} Zrušené.`
        },
        title: () => `☄️ Pozvánky synchronizované`,
        titles: {
            all: () => `${emojis.success} | Pozvánky servera synchronizované!`
        }
    },

    add: {
        content: (id) => `Môžeš ma pridať do svojho servera kliknutím [tu](https://discordapp.com/oauth2/authorize?client_id=${id}&scope=bot&permissions=2146958847).`,
        requested: (username) => `Požiadal ${username}`
    },

    support: {
        content: (id) => `Môžeš sa pripraviť na náš discord server kliknutím [tu](https://discord.gg/f7f2P2q).`,
        requested: (username) => `Požiadal ${username}`
    }
};
