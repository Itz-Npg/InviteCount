...
module.exports = {
    ...
    utils: {
        ...
        specialMessages: {
            join: {
                ...
                vanity: (user) => `${user} entrou no servidor usando o convite discord.gg definido pelo dono do servidor.`,
                unknown: (user) => `Não consegui entender como ${user} entrou no servidor.`,
                perm: (user) => `${emojis.error} | Preciso de permissão para gerenciar o servidor para saber quem convidou ${user}.`
            },
            leave: {
                oauth2: (user) => `${user} saiu do servidor, tinha entrado via OAuth.`,
                vanity: (user) => `${user} saiu do servidor, tinha entrado via convite discord.gg definido pelo dono do servidor.`,
                unknown: (user) => `${user} saiu do servidor, mas não consegui entender como ele entrou.`
            }
        }
    },

    errors: {
        missingPerms: (neededPermissions) => `__**${emojis.error} Permissões faltando**__\n\nPreciso das seguintes permissões para esse comando funcionar corretamente: ${neededPermissions.map((p) => "`"+p+"`").join(", ")}`,
        disabled: () => `${emojis.error} | Esse comando está desabilitado no momento!`,
        permLevel: (name) => `${emojis.error} | Esse comando requer nível de permissão: \`${name}\`!`,
        sendPerm: () => `${emojis.error} | Não tenho permissão para enviar mensagens neste canal.`,
        missingAdmin: () => `${emojis.error} | Preciso da permissão \`Gerenciar Servidor\` para executar essa ação`
    },
    glogs: {
        err: () => `<:error:851490719934840872> | O canal atual já é esse canal!`,
        success: () => `<:succes:851491428563812382> | Canal de logs definido com sucesso.`
    },
    glist: {
        title: () => `🎁 Lista de Sorteios`,
        description: (prefix) => `Use \`${prefix}gstart\` para iniciar um sorteio. Apenas sorteios não concluídos são mostrados`,
        fields: {
            name: () => `> Lista`,
    ...
    help: {
        ...
        // Others
        tip: (prefix) => `🔗 Links :`,
        links: (clientID) => `[Me Adicione](https://discordapp.com/api/oauth2/authorize?client_id=${clientID}&permissions=8&scope=bot) ● [Site](https://invite-count.xyz) ● [Servidor de Suporte](${discord}) ● [Vote em Mim](https://discord.boats/bot/${clientID}/vote)`
    },
    ...
    leaderboard: {
        ...
        prompt: () => `{{user}}, em qual página você quer ir? Digite \`cancel\` ou \`0\` para cancelar.`,
        title: () => `Placar de Convites`,
        empty: {
            title: () => `😕 Nenhum convite encontrado`,
            content: () => `Comece a convidar pessoas e você vai aparecer nessa página!`
        }
    },
    ...
    userinfo: {
        ...
        fields: {
            ...
            joinWay: {
                ...
                unknown: (user) => `Não consegui entender como ${user.username} entrou no servidor.`,
    ...
    kick: {
        errors:{
            missingPerms: "<:error:753232040199192657> | Você precisa ter a permissão `KICK_MEMBERS`",
            ...
            noperm: "<:error:753232040199192657> | Um erro ocorreu... Por favor, verifique se tenho permissão para expulsar esse membro e tente novamente!",
    ...
    ban: {
        errors:{
            missingPerms: "<:error:753232040199192657> | Você precisa ter a permissão `BAN_MEMBERS`",
            nouser: "<:error:753232040199192657> | Por favor, mencione um usuário",
            supperior: "<:error:753232040199192657> | Você não pode banir um usuário superior a você",
            noperm: "<:error:753232040199192657> | Um erro ocorreu... Por favor, verifique se tenho permissão para banir esse membro e tente novamente!",
    ...
    unban: {
        ...
        noban: "<:error:753232040199192657> | Esse usuário não está banido"
    },
    ...
    addbonus: {
        errors: {
            bonus: {
                missing: (prefix) => `${emojis.error} | Você precisa especificar o número de convites bônus para adicionar. (Sintaxe: ${prefix}addbonus número @membro) **Você precisa convidar uma pessoa real para ganhar uma recompensa**`,
                incorrect: (prefix) => `${emojis.error} | Você precisa especificar um número válido de convites bônus para adicionar. (Sintaxe: ${prefix}addbonus número @membro)`
            },
            member: {
                missing: (prefix) => `${emojis.error} | Você precisa mencionar um membro para adicionar convites bônus. (Sintaxe: ${prefix}addbonus número @membro)`
            }
        },
        title: () => `📥 Convites Bônus Adicionados`,
        field: (prefix, member) => `Digite \`${prefix}invites ${member.user.tag}\` para ver a quantidade nova de convites de **${member.user.username}**!`
    },

    removebonus: {
        errors: {
            bonus: {
                missing: (prefix) => `${emojis.error} | Você precisa especificar o número de convites bônus para remover. (Sintaxe: ${prefix}removebonus número @membro)`,
                incorrect: (prefix) => `${emojis.error} | Você precisa especificar um número válido de convites bônus para remover. (Sintaxe: ${prefix}removebonus número @membro)`
            },
            member: {
                missing: (prefix) => `${emojis.error} | Você precisa mencionar um membro para remover convites bônus. (Sintaxe: ${prefix}removebonus número @membro)`
            }
        },
    ...
    setdmjoin: {
        on: () => `**${emojis.success} | Sistema de Entrada por DM agora está __ATIVADO__!**`,
        off: () => `**${emojis.success} | Sistema de Entrada por DM agora está __DESATIVADO__!**`
    },
    ...
    setprefix: {
        missing: () => `${emojis.error} | Você precisa especificar um prefixo!`,
        success: () => `${emojis.success} | Prefixo do servidor atualizado!`
    },

    testdmjoin: {
        title: () => `:wrench: Sistema de Entrada por DM :`,
        description: () => `Se não tiver funcionando, verifique as permissões do bot ou entre no [servidor de suporte](${discord})`,
        fields: {
            enabled: () => `> Ativado:`,
            message: () => `> Mensagem:`
        },
        enabled: (prefix) => `${emojis.success} Mensagens de entrada por DM ativadas. Desative com \`${prefix}setdmjoin\`.`,
        disabled: (prefix) =>  `${emojis.error} Mensagens de entrada por DM desativadas. Ative com \`${prefix}setdmjoin\`.`,
        notDefineds: {
    ...
    config: {
        ...
        joinDM: {
            title: (enabled) => `${(enabled ? emojis.success : emojis.error)} Mensagens de Entrada por DM`,
            content: (guild, data) => `
    ...
    joinDM: {
        premium: (username) => `:crown: | Oi, **${username}**! Esse recurso está disponível apenas para servidores premium e parceiros. Pega seu premium: **em breve** !`
    },

    antispam: {
        cooldown: (username) => `Vai com calma! Por favor, aguarde 5 segundos e tente novamente!`
    },

    configdmjoin: {
        disable: (prefix) => `Digite \`${prefix}setdmjoin\` para desativar as mensagens de entrada por DM.`,
        instruct: (str) => `
...
:pencil: **| Agora escreva a mensagem de entrada por DM... :pencil2:**`,
        cancelled: () => `:x: Cancelado.`,
        success: () => `✅ **| Concluído com sucesso...**`,
        title: () => `**Mensagem de Entrada por DM Definida**`,
        fields: {
            message: () => `Mensagem:`,
            testIt: () => `Teste:`,
    ...
    configjoin: {
        longmessage: () => `<:error:753232040199192657> | Sua mensagem precisa ter menos de 850 caracteres.`, 
...
:pencil: **| Agora escreva a mensagem de saída... :pencil2:**`,
    ...
    setlang: {
        invalid: () => `${emojis.error} | Você precisa especificar um idioma válido!\n\n:flag_fr: Français (\`fr\`)\n:flag_gb: English (\`en\`)\n:flag_tr: Türkçe (\`tr\`)\n:flag_ru: Русский (\`ru\`)\n:flag_jp: 日本語 (\`jp\`)\n:flag_in: हिंदी (\`hi\`)\n:flag_pt: Português (\`pt\`)\n:flag_br: Português Brasileiro (\`br\`)\n:flag_es: Español (\`es\`)\n:flag_de: Deutsch (\`de\`)\n:flag_it: Italiano (\`it\`)\n:flag_pl: Polski (\`pl\`)\n:flag_nl: Nederlands (\`nl\`)`,
        success: () => `${emojis.success} | :flag_br: O idioma do servidor agora é Português Brasileiro!`
    },
    ...
    graph: {
        ...
        invalid: () => `Você precisa inserir um número válido de dias (maior que 1 e menor que 1000) para ser mostrado!`,
        months: () =>[
            "Jan",
            "Fev",
            "Mar",
    ...
    website: {
        ...
        forms: {
            ...
            joinDM: {
                title: () => `Mensagens de Entrada por DM`,
                premium: () => `Esse recurso está disponível para servidores premium e parceiros.`,
                message: {
                    title: () => `Mensagem`,
                    default: () => `Bem-vindo {user}! Você entrou em **{server}**! Convidado por **{inviter.tag}**. Não se esqueça de ler as regras do servidor!`,
                }
            }
        }
    },
    ...
    removeinvites: {
        loading: {
            all: (prefix) => `${emojis.loading} | Processo de remoção de convites do servidor foi iniciado... Você pode restaurá-los com o comando \`${prefix}restore-invites\`!`,
            member: (prefix, member) => `${emojis.loading} | Processo de remoção de convites de **${member.user.tag}}** foi iniciado... Você pode restaurá-los com o comando \`${prefix}restore-invites ${member.user.tag}\`!`
        },
        title: () => `☄️ Convites Redefinidos`,
        titles: {
            all: (prefix) => `${emojis.success} | Convites do servidor foram redefinidos! Você pode restaurá-los com o comando \`${prefix}restore-invites\`!`,
            member: (prefix, member) => `${emojis.success} | Convites de **${member.user.tag}}** foram redefinidos! Você pode restaurá-los com o comando \`${prefix}restore-invites ${member.user.tag}\`!`
        }
    },

    restoreinvites: {
        confirmations: {
            all: (prefix, memberCount) => `${emojis.warn} | Tem certeza que quer restaurar os convites do servidor? Todos os membros receberão os convites de antes da última execução do comando \`${prefix}remove-invites\` (ou 0 se o comando nunca foi executado).\n\n:information_source: **Resumo dos convites**:\nSerão restaurados, total: **${memberCount.invites}** normais, **${memberCount.bonus}** bônus, **${memberCount.leaves}** saídas, **${memberCount.fake}** falsos.\n\n${emojis.success} Digite \`-confirm\` para confirmar.\n${emojis.error} Digite \`cancel\` para cancelar.`,
            member: (prefix, member) => `${emojis.warn} | Tem certeza que quer restaurar os convites de **${member.user.tag}}**? Receberá os convites de antes da última execução do comando \`${prefix}remove-invites\` (ou 0 se o comando nunca foi executado).\n\n:information_source: **Resumo dos convites**:\nSerão restaurados: **${member.data.old_invites}** normais, **${member.data.old_bonus}** bônus, **${member.data.old_leaves}** saídas, **${member.data.old_fake}** falsos.\n\n${emojis.success} Digite \`-confirm\` para confirmar.\n${emojis.error} Digite \`cancel\` para cancelar.`,
            cancelled: () => `${emojis.error} Cancelado.`
        },
        loading: {
            all: () => `${emojis.loading} | Processo de restauração de convites do servidor foi iniciado....`,
            member: (member) => `${emojis.loading} | Processo de restauração de convites de **${member.user.tag}}** foi iniciado...`
        },
        title: () => `☄️ Convites Restaurados`,
        titles: {
            all: () => `${emojis.success} | Convites do servidor foram restaurados!`,
            member: (member) => `${emojis.success} | Convites de **${member.user.tag}}** foram restaurados!`
        }
    },
    ...
    syncinvites: {
        ...
        confirmations: {
            all: (inviteCount) => `${emojis.warn} | Tem certeza que quer sincronizar os convites do servidor?\n\n:information_source: **Resumo dos convites**:\n**${Math.round(inviteCount)}** convites normais serão restaurados.\n\n${emojis.success} Digite \`-confirm\` para confirmar.\n${emojis.error} Digite \`cancel\` para cancelar.`,
            cancelled: () => `${emojis.error} Cancelado.`
        },
        title: () => `☄️ Convites Sincronizados`,
        titles: {
            all: () => `${emojis.success} | Convites do servidor foram sincronizados!`
        }
    },

    add: {
        content: (id) => `Você pode me adicionar no seu servidor clicando [aqui](https://discordapp.com/oauth2/authorize?client_id=${id}&scope=bot&permissions=2146958847).`,
