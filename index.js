const { Message, WebhookClient, EmbedBuilder, ActivityType } = require("discord.js");
...
const init = async () => {
    if (await client.waitForDb()) {
        try {
            let a = await client.db.get('giveaways');
            if (a === null) await client.db.set('giveaways', []);
        } catch (err) {
            console.error('Error initializing giveaways:', err.message);
        }
    }
    ...
    if (client.config.mongodb) {
        mongoose.connect(client.config.mongodb).catch((err) => {
            client.logger.log("Unable to connect to the Mongodb database. Error:"+err, "error");
        });
    } else {
        console.warn('WARNING: MONGODB_URI not set. Some features requiring MongoDB will not work.');
    }
    ...
    client.on("shardReady", (shardID) => {
        ...
        try {
            const shardlog = new WebhookClient({ id: '756874584149459065', token: '**' });
            const postMessage = `<:online:757044247269605386> | Shard #${shardID} is ready!`;
            shardlog.send(postMessage).catch(() => {});
        } catch (e) {
